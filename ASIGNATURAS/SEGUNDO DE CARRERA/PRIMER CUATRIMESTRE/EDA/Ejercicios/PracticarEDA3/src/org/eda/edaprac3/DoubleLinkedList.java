...
public class DoubleLinkedList<T> {
	...
	public void borrarLista(DoubleLinkedList<T> subLista) {
		...
		if (subLista.first == null) {
			...
			DoubleNode actual=this.first;
			boolean enc=false;
			while(!enc&&actual!=this.first.prev) {
				if(actual.data!=subLista.first.data) actual=actual.next;
				else enc=true;
			}	
			DoubleNode actualaux=subLista.first;
			while(actual.data.equals(actualaux.data)) {
				actual.prev.next=actual.next;
				actual.next.prev=actual.prev;
				actual.prev=null;
				if(actual==this.first) {
					this.first=actual.next;
					if(actual.next==actual) this.first=null;
				}
				actual=actual.next;
