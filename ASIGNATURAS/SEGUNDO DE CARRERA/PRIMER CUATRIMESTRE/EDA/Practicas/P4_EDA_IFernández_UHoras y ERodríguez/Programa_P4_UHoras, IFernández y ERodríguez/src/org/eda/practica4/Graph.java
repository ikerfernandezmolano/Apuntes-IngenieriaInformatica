...
public class Graph {
    ...
    public HashMap<String, Double> calcularRandomWalkRank(int nTests) {
		...
		System.out.println("\nEL MÉTODO calcularRandomWalkRank HA TARDADO "+String.format("%.4f", tiempo)+" SEGUNDOS EN EJECUTARSE");
   ...
   public HashMap<String, Double> calcularPageRank() {
		...
		double tiempo=reloj.elapsedTime();
		System.out.println("\nEL MÉTODO calcularPageRank HA TARDADO "+String.format("%.4f", tiempo)+" SEGUNDOS EN EJECUTARSE");
		return resultado;
   }
   
   public HashMap<String, Double> calcularRandomWalkRank1(int nTests) {
		double d = 0.85; // damping factor
		Random r = new Random();
		// COMPLETAR CÓDIGO
		int n = enlaces.size(); //Número de webs
		...
		for(int j=0;j<nTests;j++) {
			int x = r.nextInt(n); //El número de web desde la que empezamos el recorrido 
			Iterator<String> it = enlaces.keySet().iterator();
			int cont=0;
			String web = null;
			while(cont<x /*&& it.hasNext()*/) {
				web = it.next();
				cont++;
			}
			//En el String web está guardada la web desde la que vamos a empezar el recorrido
			
			//HashSet<String> examinadas = new HashSet<>();
			ArrayList<String> examinadas = new ArrayList<>();
			Queue<String> porExaminar = new LinkedList<>();
			porExaminar.add(web);
			boolean finRecorrido = false;
			while(!finRecorrido && !porExaminar.isEmpty()) {
				String w = porExaminar.remove();
				if(!examinadas.contains(w)) {
					examinadas.add(w);
					double valor = m.get(w);
					m.put(w, valor+1);
					/*for(int i=0;i<enlaces.get(w).size() && !finRecorrido;i++) {
						double p = r.nextInt(101)/100.0;
						if(p>0.85) {
							finRecorrido = true;
						}else {
							porExaminar.add(enlaces.get(w).get(i)); 
						}
					}*/
					if(enlaces.get(w).size()==0) {
						finRecorrido = true;
					}else {
						int posEnlace = r.nextInt(enlaces.get(w).size());
						double p = r.nextInt(101)/100.0;
						
						if(p>0.85) {
							finRecorrido = true;
						}else {
							porExaminar.add(enlaces.get(w).get(posEnlace)); 
						}
					}
				}else {
					finRecorrido = true;
   ...
   public HashMap<String, Double> calcularPageRank1() {
   ...
   public void imprimirLosDeMejorPageRank(HashMap<String, Double> pMapa, int n) {
       ...
       for (Map.Entry<String, Double> entry : result) {
           System.out.println("La web en la posición de importancia "+ ++i+" es: " + entry.getKey() + ", con un valor de: " + String.format("%.6f", entry.getValue()));
       }
   }
}
